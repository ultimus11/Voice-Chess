...
def rooks(move_possible,turn,xval_r,xval,yval_r,yval,destroy):
	...
	if move_possible==1 and turn == 1:
		try:
			#now we will check if required position is valid : : :  This is for rooks remember destryoing condition is not given
			#find traversable positions
			traversable_pos = []
			for position_key in All_positions_y:
				if continue_traversing == True:
					value = All_positions_y[position_key]
					if xval!=xval_r and value != yval_r:
						continue_traversing = check_key(xval,value)
					if continue_traversing == False:
						break
					if xval==xval_r and value == yval_r:
						continue_traversing = check_w_army_key(xval,value)
					if continue_traversing == False:
						break
					if xval==xval_r and value == yval_r:
						continue_traversing = check_b_army_key(xval,value)
						if continue_traversing == False:
							traversable_pos.append([xval,value])
							break
					traversable_pos.append([xval,value])
			w_army_values = []
			for elements in white_army:
				values = white_army[elements]
				w_army_values.append(values)
			print("printing w army", w_army_values)
			stop =0
			new_traversel =[]
			print("traversable orig poss1",traversable_pos)
			for idx, elements in enumerate(traversable_pos):
				if elements == [xval,yval]:
					del traversable_pos[idx]
			print("new pos",traversable_pos)
			for elements in traversable_pos:
				for army_elements in w_army_values:
					if elements[0]==army_elements[0] and elements[1]==army_elements[1]:
						print(elements)
						print(army_elements)
						stop=1
						break
				if stop == 1:
					break
				new_traversel.append(elements)
			print(new_traversel)
			traversable_pos=new_traversel.copy()
			print(traversable_pos)

			continue_traversing =True
			for position_key in All_positions_x:
				if continue_traversing == True:
					value = All_positions_x[position_key]
					if value!=xval_r and yval != yval_r:
						continue_traversing = check_key(value,yval)
					if value==xval_r and yval == yval_r:
						continue_traversing = check_w_army_key(value,yval)
					if continue_traversing == False:
						break
					if value==xval_r and yval == yval_r:
						continue_traversing = check_b_army_key(value,yval)
					if continue_traversing == False:
						traversable_pos.append([value,yval])
						break
					traversable_pos.append([value,yval])
					#traversable_pos.append([value,yval])
			w_army_values = []
			...
			print("printing w army", w_army_values)
			new_traversel =[]
			print("traversable orig poss2",traversable_pos)
			for idx, elements in enumerate(traversable_pos):
				if elements == [xval,yval]:
					del traversable_pos[idx]
			print("new pos",traversable_pos)
			...
			print(traversable_pos)
			validation =False
			for lists in traversable_pos:
				print(lists)
				if xval_r==lists[0] and yval_r == lists[1]:
					validation = True
			print("validation", validation)
		except ValueError:
			validation = False
